 ...
 **************************************************************************/
#include "quash.h" 
// Putting this above the other includes allows us to ensure
...
void run_cmd(char *cmd) {
    ...
    /*Different from system() and exec(),
     *it returns the new child process’ pid which you can wait by waitpid().
     *Of course, system() is something you should forget that you have learned it in most situations.
     *Difference from fork()/vfork(),
     *the logic you implement is within the same process
     *and you do not need to think about which piece of code is executed by the child process
     *and which is executed by the parent process. It also avoid problems from vfork().*/
